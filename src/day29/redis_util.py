...
class TestRedis:
    ...
    def openRedis(self):
        ...
        pool = redis.ConnectionPool(host='172.31.242.161', port=6370, db=1, password='G_8NxEfcuyOmqMg', decode_responses=True)  # 使用连接池
    ...
    def nxAandxxDemo(self):
        ...
        # 此时是 zhangsan 并没有改为 一个
        print(self.getKey("grape"))
        # 此时是 一个2
        print(self.getKey("grape2"))
        print("========================")
        # grape2已经存在
        self.setObj("grape2", "修改了", xx=True)
        # 此时是 修改了
        print(self.getKey("grape2"))
        # grape3不存在 不会创建
        self.setObj("grape3", "修改了", xx=True)
        # None
        print(self.getKey("grape3"))
